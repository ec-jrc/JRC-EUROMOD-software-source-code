...
namespace InDepthAnalysis
{
    internal partial class SettingsFiscal : Form, ISettings
    {
        ...
        void ISettings.ModifyTemplate(TemplateApi templateApi, out List<Template.TemplateInfo.UserVariable> systemSpecificVars)
        {
            ...
            if (!settings.inactiveTablesAndPages.Contains(tableName_RevenueExpenditure))
            {
                ...
                if (settings.baselineReformPackages.Count == 1) // no incomelist-component-break-down if more than one country is analysed
                {
                    ...
                    foreach (string il in new List<string> { SystemInfo.ILS_EARNS, SystemInfo.ILS_TAX, SystemInfo.ILS_SICCT, SystemInfo.ILS_SICDY, SystemInfo.ILS_SICER,
                                                             SystemInfo.ILS_PEN, SystemInfo.ILS_BENMT, SystemInfo.ILS_BENNT })
                    {
            ...
            if (!settings.inactiveTablesAndPages.Contains(tableName_FiscalTaxpayersBeneficiaries))
            {
                ...
                foreach (string il in new List<string> { SystemInfo.ILS_PEN, SystemInfo.ILS_BENMT, SystemInfo.ILS_BENNT })
                {
        ...
        {
            if (gridDisCon.SelectedRows.Count == 0) return;
            int iSelRow = gridDisCon.Rows.IndexOf(gridDisCon.SelectedRows[0]); if (iSelRow == 0) return;
            List<object> rowContent = new List<object>(); foreach (DataGridViewCell cell in gridDisCon.SelectedRows[0].Cells) rowContent.Add(cell.Value);
            gridDisCon.Rows.Insert(iSelRow - 1, rowContent.ToArray()); gridDisCon.Rows.RemoveAt(iSelRow + 1);
            gridDisCon.ClearSelection(); gridDisCon.Rows[iSelRow - 1].Selected = true;
        }

        private void btnDisConMoveDown_Click(object sender, EventArgs e)
        {
            if (gridDisCon.SelectedRows.Count == 0) return;
            int iSelRow = gridDisCon.Rows.IndexOf(gridDisCon.SelectedRows[0]); if (iSelRow == gridDisCon.Rows.Count - 1) return;
            List<object> rowContent = new List<object>(); foreach (DataGridViewCell cell in gridDisCon.SelectedRows[0].Cells) rowContent.Add(cell.Value);
        ...
        private void txtTaxpayersBeneficiaries_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            {
                MessageBox.Show($"Please indicate a condition for {(textBox == txtTaxpayers ? "Taxpayers" : "Beneficiaries")}");
                e.Cancel = true;
            }
            else e.Cancel = !Settings.IsValidFormula(textBox.Text, true, true);
        }

        private void gridDisCon_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            if (e.RowIndex < 0) return;
            if (string.IsNullOrEmpty(e.FormattedValue?.ToString()) && (e.ColumnIndex == colDisConFormula.Index || e.ColumnIndex == colDisConTaxpayersBeneficiaries.Index))
            {
                MessageBox.Show($"{(e.ColumnIndex == colDisConFormula.Index ? "Variable/Formula" : "Taxpayers/Beneficiaries")} must not be empty."); e.Cancel = true; return;
            }
            bool isFormula = e.ColumnIndex == colDisConFormula.Index || e.ColumnIndex == colDisConFormulaFilter.Index || e.ColumnIndex == colDisConTaxpayersBeneficiaries.Index;
            bool isFilter = e.ColumnIndex == colDisConFormulaFilter.Index || e.ColumnIndex == colDisConTaxpayersBeneficiaries.Index;
            bool startsWithComparer = e.ColumnIndex == colDisConTaxpayersBeneficiaries.Index;
            if (isFormula && !Settings.IsValidFormula(e.FormattedValue?.ToString(), isFilter, startsWithComparer)) e.Cancel = true;
        }
    }
}
