...
{
    public partial class SelectBaselinesReforms : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            string basePath = txtBaselinesPath.Text;
		if (Path.GetFileName(basePath).Contains("*") || Path.GetFileName(basePath).Contains("?")) basePath = Path.GetDirectoryName(basePath);
            string reformPath = txtReformsPath.Text;
            if (Path.GetFileName(reformPath).StartsWith("*") || Path.GetFileName(reformPath).StartsWith("?")) reformPath = Path.GetDirectoryName(reformPath);
            foreach (string baseline in listBaselines.SelectedItems)
            {
                FilePackageContent fpc = new FilePackageContent() { PathBase = Path.Combine(basePath, baseline) };
                ...
                else
                {
                    ...
                    foreach (string reform in selectedIndices.Select(i => listReforms.Items[i]))  // get alt selections in order
                        if (reform.StartsWith(blPrefix, true, null)) fpc.PathsAlt.Add(Path.Combine(reformPath, reform));
                    if (!fpc.PathsAlt.Any())
                    {
