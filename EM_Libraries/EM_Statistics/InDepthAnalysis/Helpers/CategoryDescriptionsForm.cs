...
namespace EM_Statistics.InDepthAnalysis
{
    ...
    {
        internal CategoryDescriptionsForm(List<Tuple<double, string>> categoryDescriptions, bool readOnly = false, bool showValues = true)
        {
            InitializeComponent();
            ...
            if (readOnly)
            {
                ...
                btnOK.Visible = false;
                grid.ReadOnly = true;
                colValue.Visible = showValues;
            }

            if (categoryDescriptions != null)
                foreach (Tuple<double, string> cd in categoryDescriptions) grid.Rows.Add(cd.Item1, cd.Item2);
        }
        ...
        internal Dictionary<double, string> GetDescriptionsDictionary()
        {
        ...
        internal List<Tuple<double, string>> GetDescriptionsList()
        {
            List<Tuple<double, string>> descriptions = new List<Tuple<double, string>>();
            foreach (DataGridViewRow row in grid.Rows)
                if (CheckRow(row, out double v, out string d, out _)) descriptions.Add(new Tuple<double, string>(v, d));
            return descriptions;
        }
