...
namespace EM_Statistics.InDepthAnalysis
{
    public class Settings
    {
        ...
        public List<BaselineReformPackage> baselineReformPackages = new List<BaselineReformPackage>();
        ...
        public string pathEuromodFiles = UISessionInfo.GetEuromodFilesFolder();
        public string pathBaselineFiles = UISessionInfo.GetOutputFolder();
        ...
        internal class TextBoxInfo
        {
            ...
            internal bool? startsWithComparer = null;

            public TextBoxInfo(TextBox _textbox, bool _isFilter, bool? _startsWithComparer = null)
            {
                textbox = _textbox;
                isFilter = _isFilter;
                startsWithComparer = _startsWithComparer;
            }
        }

        internal class DataGridViewInfo
        {
            internal DataGridView dgv = null;
            internal bool isFilter;
            internal bool? startsWithComparer;

            public DataGridViewInfo(DataGridView _dgv, bool _isFilter, bool? _startsWithComparer = null)
            {
                dgv = _dgv;
                isFilter = _isFilter;
                startsWithComparer = _startsWithComparer;
        ...
        internal bool GetFilePackagesMacrovalidation(out List<FilePackageContent> filePackages, out List<string> errors)
        {
            filePackages = new List<FilePackageContent>(); errors = new List<string>();
            foreach (BaselineReformPackage baselineReformPackage in baselineReformPackages)
            {
                if (baselineReformPackage.GetFilePackageContentMacrovalidation(out FilePackageContent fpc, out List<string> e)) filePackages.Add(fpc);
                errors.AddRange(e);
            }
            if (!filePackages.Any()) errors.Add($"No valid baseline-reform packages available.");
            return filePackages.Any();
        ...
        {
            using (XmlWriter xmlWriter = XmlWriter.Create(xmlFilePath, new XmlWriterSettings () { Indent = true }))
            {
                                                 ...
                                                 out string handledFormula, out List<Template.Parameter> parameters)
        {
            ...
            if (!handledFormula.Contains(HardDefinitions.FormulaParameter.DATA_VAR))
            {
                ...
                if (Double.TryParse(handledFormula, out double d)) { handledFormula = d.ToString("F", CultureInfo.InvariantCulture); return true; }
        ...
        {
            return IsValidFormula(formula, isFilter, out _, startsWithComparer, true);
        }

        internal static bool IsValidFormula(string formula, bool isFilter, out string error, bool startsWithComparer = false, bool showMessage = false)
        {
            error = string.Empty;
            ...
            if (startsWithComparer)
            {
                ...
                if (!ok) { 
                    error = $"Invalid condition: must start with one of {string.Join("  ", cmps)}"; 
            ...
            if (TemplateApi.IsValidFormula(formula, isFilter, out error)) return true;
            if (showMessage) MessageBox.Show(error);
            return false;
        }
